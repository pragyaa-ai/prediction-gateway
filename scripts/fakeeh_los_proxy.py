...
from los_encoder import LOSDataEncoder  # type: ignore[reportMissingImports]  # noqa: E402 — deployed beside this script
...
def invoke_local_gateway_sync(final_payload: dict) -> Optional[float]:
    payload = {"client_id": "fakeeh-flask-los-proxy", "inputs": final_payload}
    response = requests.post(
        LOCAL_GATEWAY_URL,
        json=payload,
...
def get_length_of_stay():
    try:
        ...
        if USE_LOCAL_PRIMARY:
            logging.info("Calling local gateway (primary): %s", LOCAL_GATEWAY_URL)
            try:
                predicted_los = invoke_local_gateway_sync(final_payload)
            except Exception:
                logging.exception("Local LOS gateway (primary) failed")
                predicted_los = None
        ...
        if predicted_los is None and CLOUD_ENABLED:
            ...
            if resp.status_code == 200:
                ...
                if "body" in outer:
                    ...
                    predicted_los = outer.get("Predicted_length_of_stay")
            else:
        ...
        if predicted_los is None:
            return jsonify({"error": "No prediction from local or cloud"}), 500
